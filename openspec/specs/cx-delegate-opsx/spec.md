# cx-delegate-opsx Spec

## Purpose

Delegate bounded task groups from an OpenSpec change to the Codex CLI for implementation, then review the diff and land it. The orchestrator constructs a prompt from the change's spec, design, and tasks, and pipes it directly to `codex exec` without a relay script.
...
After implementing, the Codex sub-agent SHALL mark completed tasks in `tasks.md` by changing `- [ ]` to `- [x]`. It SHALL NOT edit design.md or spec files — the orchestrator handles those after review.
...
The orchestrator SHALL capture the Codex thread ID from the CLI's JSON output stream, not from sub-agent self-report. The orchestrator SHALL redirect stderr to a file with a fixed naming convention and parse the session ID from it after the run completes.
...
- **THEN** the orchestrator extracts `thread_id` from the JSON event stream after completion
