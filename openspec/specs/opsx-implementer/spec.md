# opsx-implementer Spec

## Purpose

Delegate bounded task groups from an OpenSpec change to a CLI coding agent (OpenCode or Codex) for implementation, then review the diff and land it. The orchestrator constructs a prompt from the change's spec, design, and tasks, and pipes it directly to the chosen implementer CLI.
