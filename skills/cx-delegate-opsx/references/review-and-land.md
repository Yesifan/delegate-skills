...
- If it needs changes, resume the session with a delta prompt — don't restate the whole task:
  ...
  `resume` keeps the implementer's context from the first run (and its model), so a short delta is
  enough. Then review again — rework gets the same test check, gate re-run, diff-read, and sweep as
